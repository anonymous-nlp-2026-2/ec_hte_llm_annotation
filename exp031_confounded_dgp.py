#!/usr/bin/env python3
"""exp031_confounded_dgp.py — Confounded DGP experiment (R1)

Validates EC-HTE under confounded treatment assignment:
T = Bernoulli(sigmoid(beta * X + noise)) instead of Bernoulli(0.5).
Uses IPW-DIM to handle confounding, then EC-HTE to handle misclassification.
Includes Oracle (true Z + IPW) as upper bound.

Output: results/exp_confounded_dgp.csv, results/exp_confounded_dgp_summary.csv
"""
...
SUB_WEIGHTS = [0.5, 0.5]
N_MC = 100
BETA_CONFOUND = 0.5  # moderate confounding
...
def generate_data(seed):
    ...
    propensity = expit(BETA_CONFOUND * X_conf)
    ...
    return Y, T, Z_true, Z_hat, S, expert_mask, propensity
...
def ipw_dim_estimate(Y, T, Z_labels, S, s, propensity):
    """IPW-adjusted DIM estimator with known propensity."""
    mask_s = S == s
    results = {}
    for z in [0, 1]:
...
def run_one_mc(seed):
    Y, T, Z_true, Z_hat, S, expert_mask, propensity = generate_data(seed)
...
def main():
    ...
    print(f"Confounded DGP: K={K}, N={N}, n_expert={N_EXPERT}")
    print(f"beta_confound={BETA_CONFOUND}")
    print(f"PI={PI}, TAU_Z={TAU_Z}")

    # Show propensity distribution
    rng = np.random.RandomState(42)
    X_test = rng.normal(0, 1, N)
    prop_test = expit(BETA_CONFOUND * X_test)
    print(f"Propensity: mean={prop_test.mean():.3f}, std={prop_test.std():.3f}, "
          f"range=[{prop_test.min():.3f}, {prop_test.max():.3f}]")
    ...
    df.to_csv('results/exp_confounded_dgp.csv', index=False)
    print(f"\nWrote {len(df)} rows")
    ...
    summary.to_csv('results/exp_confounded_dgp_summary.csv', index=False)

    print("\n" + "=" * 60)
    print(f"CONFOUNDED DGP — IPW-DIM (known propensity)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PER-SUBGROUP (s0=low-pi, s1=high-pi)")
    print("=" * 60)
    for method in ['Oracle (true Z)', 'Naive', 'Global', 'HB EC-HTE']:
        m_data = sub_summary[sub_summary['method'] == method].sort_values('subgroup')
        for _, r in m_data.iterrows():
            print(f"  {method:>18} s{int(r['subgroup'])}: "
                  f"|bias|={r['mean_abs_bias']:.4f} rmse={r['rmse']:.4f} cov={r['coverage']:.3f}")
