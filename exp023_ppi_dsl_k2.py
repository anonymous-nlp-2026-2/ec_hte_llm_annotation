"""
exp-023 K=2 variant: PPI/DSL baseline comparison with K=2 extreme subgroups.

K=2 extreme: π=[0.05, 0.20], DELTA_S=[0.4, 0.8]
Subgroups defined by has_hashtag only (S0=no hashtag, S1=has hashtag).
"""
...
MISCLASS_RATES = {
    'extreme': {'S0': 0.05, 'S1': 0.20},
}

DELTA_S = {'S0': 0.4, 'S1': 0.8}
TAU_Z0 = 0.1
ALPHA = 1.0
BETA_LENGTH = 0.3
BETA_MENTION = 0.2
...
def assign_subgroups_k2(has_hashtag):
    S = np.where(has_hashtag == 0, 'S0', 'S1')
    return S


# === Core Estimators (same as exp023) ===
...
def generate_data(pool_texts, pool_labels, pool_has_hashtag, pool_text_lengths,
    ...
    S = assign_subgroups_k2(has_hashtag)
...
if __name__ == '__main__':
    ...
    print(f"exp-023 K=2: PPI/DSL Baseline Comparison (K=2 extreme)")
    print(f"N={N}, N_MC={n_mc}, N_EXPERTS={N_EXPERTS}")
    print(f"Subgroups: {SUBGROUP_LABELS}, misclass: {MISCLASS_RATES['extreme']}")
    ...
    raw_path = 'results/exp023_ppi_dsl_k2_raw.csv'
    agg_path = 'results/exp023_ppi_dsl_k2.csv'
    ...
    # Table 1: Subgroup-averaged metrics
    for regime in MISCLASS_RATES:
        pi = MISCLASS_RATES[regime]
        print(f"\n{'='*160}")
        print(f"TABLE 1: Subgroup-Averaged Metrics | Regime: {regime} | K=2 | misclass: {pi}")
        print(f"{'='*160}")

        header = f"{'Method':<42}"
        for ne in N_EXPERTS:
            header += f" | n={ne}: {'|Bias|':>7} {'RMSE':>7} {'Cov':>5}"
        print(header)
        print("-" * len(header))
        ...
        for method in method_order:
            ...
            for ne in N_EXPERTS:
                sub = df_agg[(df_agg['regime'] == regime) & (df_agg['n_expert'] == ne)
                             & (df_agg['level'].isin(SUBGROUP_LABELS)) & (df_agg['method'] == method)]
                if sub.empty:
                    line += f" | {'N/A':>7} {'N/A':>7} {'N/A':>5}"
                    continue
                avg_abs_bias = (sub['bias_z0'].abs().mean() + sub['bias_z1'].abs().mean()) / 2
                avg_rmse = (sub['rmse_z0'].mean() + sub['rmse_z1'].mean()) / 2
                avg_cov = (sub['coverage_z0'].mean() + sub['coverage_z1'].mean()) / 2
                line += f" | {avg_abs_bias:>7.4f} {avg_rmse:>7.4f} {avg_cov:>5.2f}"
            print(line)

    # Table 2: Per-subgroup breakdown
    for regime in MISCLASS_RATES:
        pi = MISCLASS_RATES[regime]
        print(f"\n{'='*160}")
        print(f"TABLE 2: Per-Subgroup Breakdown | Regime: {regime} | K=2")
        print(f"{'='*160}")

        for ne in N_EXPERTS:
            print(f"\n--- n_expert = {ne} ---")
            print(f"{'Method':<42} {'Subgroup':>8} {'Bias_z0':>9} {'Bias_z1':>9} "
                  f"{'RMSE_z0':>9} {'RMSE_z1':>9} {'Cov_z0':>7} {'Cov_z1':>7}")
            print("-" * 105)

            for method in method_order:
                label = method_display.get(method, method)
                for s_val in SUBGROUP_LABELS:
                    row = df_agg[(df_agg['regime'] == regime) & (df_agg['n_expert'] == ne)
                                 & (df_agg['method'] == method) & (df_agg['level'] == s_val)]
                    if row.empty:
                        continue
                    r = row.iloc[0]
                    print(f"{label:<42} {s_val:>8} {r['bias_z0']:>9.4f} {r['bias_z1']:>9.4f} "
                          f"{r['rmse_z0']:>9.4f} {r['rmse_z1']:>9.4f} "
                          f"{r['coverage_z0']:>7.2f} {r['coverage_z1']:>7.2f}")

    # EC-HTE vs Global RMSE comparison
    print(f"\n\n{'='*100}")
    print("EC-HTE vs Global Correction: RMSE Comparison (K=2 extreme)")
    print(f"{'='*100}")
    for ne in N_EXPERTS:
        for regime in MISCLASS_RATES:
            sub_ec = df_agg[(df_agg['regime'] == regime) & (df_agg['n_expert'] == ne)
                            & (df_agg['method'] == 'ec_hte') & (df_agg['level'].isin(SUBGROUP_LABELS))]
            sub_gl = df_agg[(df_agg['regime'] == regime) & (df_agg['n_expert'] == ne)
                            & (df_agg['method'] == 'global_corrected') & (df_agg['level'].isin(SUBGROUP_LABELS))]
            if sub_ec.empty or sub_gl.empty:
                continue
            ec_rmse = (sub_ec['rmse_z0'].mean() + sub_ec['rmse_z1'].mean()) / 2
            gl_rmse = (sub_gl['rmse_z0'].mean() + sub_gl['rmse_z1'].mean()) / 2
            winner = "EC-HTE" if ec_rmse < gl_rmse else "Global"
            delta = gl_rmse - ec_rmse
            print(f"  n={ne:>4} {regime}: EC-HTE RMSE={ec_rmse:.4f}, Global RMSE={gl_rmse:.4f}, "
