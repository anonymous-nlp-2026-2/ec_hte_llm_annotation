"""
exp-016: Semi-Real CM with Better Model (lower worst-subgroup FNR).

Selected model: gpt-3.5-turbo (worst-subgroup FNR=0.211, vs GPT-4.1's 0.526 in exp-015).
gpt-3.5-turbo trades higher FPR for much lower FNR across all subgroups.

4 subgroups with real asymmetric CMs (from exp-008 pilot annotations, n>=10 only):
  S0 (False_medium): FPR=0.143, FNR=0.211, weight=0.38
  S1 (False_short):  FPR=0.180, FNR=0.150, weight=0.22
  S2 (True_short):   FPR=0.393, FNR=0.154, weight=0.14
  S3 (True_medium):  FPR=0.378, FNR=0.148, weight=0.25

Output:
  results/exp016_semi_real_raw.csv
  results/exp016_semi_real_analysis.md

Usage: python3 exp016_semi_real_better_cm.py --n-mc 100
"""
...
# Real asymmetric CMs from gpt-3.5-turbo (exp-008 pilot)
SUBGROUP_CM = {
    'S0': {'fpr': 0.143, 'fnr': 0.211},  # False_medium
    'S1': {'fpr': 0.180, 'fnr': 0.150},  # False_short
    'S2': {'fpr': 0.393, 'fnr': 0.154},  # True_short
    'S3': {'fpr': 0.378, 'fnr': 0.148},  # True_medium
}

# exp-015 CMs for comparison (GPT-4.1)
EXP015_CM = {
    ...
    for s_i, s_val in enumerate(SUBGROUP_LABELS):
        ...
        for z_level in [0, 1]:
            ...
            rows.append({
                ...
                'ci_lo': ci_lo_or, 'ci_hi': ci_hi_or, 'coverage': cov_or,
            })
            ...
            rows.append({
                ...
                'ci_lo': ci_lo_nv, 'ci_hi': ci_hi_nv, 'coverage': cov_nv,
            })
            ...
            rows.append({
                ...
                'ci_lo': ci_lo_gc, 'ci_hi': ci_hi_gc, 'coverage': cov_gc,
            })
            ...
            rows.append({
                ...
                'ci_lo': ci_lo_hb, 'ci_hi': ci_hi_hb, 'coverage': cov_hb,
            })
...
def write_analysis(df_summary):
    lines = []
    lines.append("# exp-016: Semi-Real CM with Better Model\n")
    lines.append("## Model Selection\n")
    lines.append("Selected **gpt-3.5-turbo** from exp-008 pilot data (lowest worst-subgroup FNR).\n")
    lines.append("| Model | Worst-subgroup FNR | Meets threshold (<0.4) |")
    lines.append("|-------|-------------------|------------------------|")
    lines.append("| gpt-3.5-turbo | 0.211 | Yes |")
    lines.append("| gpt-4o-mini | 0.421 | No |")
    lines.append("| gpt-4.1 (exp-015) | 0.526 | No |")
    lines.append("| gpt-4-turbo | 0.550 | No |")
    lines.append("| gpt-4o | 0.579 | No |")
    lines.append("")
    lines.append("gpt-3.5-turbo trades higher FPR for much lower FNR (more aggressive positive predictions).\n")

    lines.append("## Design\n")
    lines.append("| Subgroup | Name | Weight | FPR | FNR | exp-015 FPR | exp-015 FNR |")
    lines.append("|----------|------|--------|-----|-----|-------------|-------------|")
    for i, s in enumerate(SUBGROUP_LABELS):
        cm = SUBGROUP_CM[s]
        cm15 = EXP015_CM[s]
        names = ['False_medium', 'False_short', 'True_short', 'True_medium']
        lines.append(f"| {s} | {names[i]} | {SUBGROUP_WEIGHTS[i]:.2f} | "
                     f"{cm['fpr']:.3f} | {cm['fnr']:.3f} | "
                     f"{cm15['fpr']:.3f} | {cm15['fnr']:.3f} |")
    lines.append("")
    ...
    lines.append("## Comparison with exp-015 (GPT-4.1 CM)\n")
    lines.append("### Overall RMSE by method and n_expert\n")
    lines.append("| n_expert | Method | exp-016 (gpt-3.5-turbo) | exp-015 (GPT-4.1) |")
    lines.append("|----------|--------|-------------------------|---------------------|")

    exp015_rmse = {
        100: {'naive': 0.1230, 'global_corrected': 0.2631, 'hb_ec_hte': 0.7842},
        250: {'naive': 0.1230, 'global_corrected': 0.2318, 'hb_ec_hte': 0.4113},
        500: {'naive': 0.1230, 'global_corrected': 0.1963, 'hb_ec_hte': 0.2599},
    }
    exp015_cov = {
        100: {'naive': 0.879, 'global_corrected': 0.885, 'hb_ec_hte': 0.744},
        250: {'naive': 0.879, 'global_corrected': 0.909, 'hb_ec_hte': 0.834},
        500: {'naive': 0.879, 'global_corrected': 0.922, 'hb_ec_hte': 0.890},
    }
    ...
    for ne in N_EXPERTS:
        for m in ['naive', 'global_corrected', 'hb_ec_hte']:
            sub = df_summary[(df_summary['n_expert'] == ne) & (df_summary['method'] == m)]
            if len(sub) > 0:
                rmse_016 = sub['mean_rmse'].mean()
                rmse_015 = exp015_rmse[ne][m]
                lines.append(f"| {ne} | {m} | {rmse_016:.4f} | {rmse_015:.4f} |")
    lines.append("")

    lines.append("### Overall Coverage by method and n_expert\n")
    lines.append("| n_expert | Method | exp-016 (gpt-3.5-turbo) | exp-015 (GPT-4.1) |")
    lines.append("|----------|--------|-------------------------|---------------------|")
    for ne in N_EXPERTS:
        for m in ['naive', 'global_corrected', 'hb_ec_hte']:
            sub = df_summary[(df_summary['n_expert'] == ne) & (df_summary['method'] == m)]
            if len(sub) > 0:
                cov_016 = sub['mean_coverage'].mean()
                cov_015 = exp015_cov[ne][m]
                lines.append(f"| {ne} | {m} | {cov_016:.3f} | {cov_015:.3f} |")
    lines.append("")
    ...
    for ne in N_EXPERTS:
        ...
        if len(hb_all) > 0 and len(gc_all) > 0:
            ...
            hb_cov = hb_all['mean_coverage'].mean()
            gc_cov = gc_all['mean_coverage'].mean()
            nv_cov = nv_all['mean_coverage'].mean()
            lines.append(f"- n_expert={ne}: HB RMSE={hb_rmse:.4f} (cov={hb_cov:.3f}), "
                         f"Global RMSE={gc_rmse:.4f} (cov={gc_cov:.3f}), "
                         f"Naive RMSE={nv_rmse:.4f} (cov={nv_cov:.3f})")

    report = "\n".join(lines)
    with open('results/exp016_semi_real_analysis.md', 'w') as f:
        f.write(report)
    print("Wrote results/exp016_semi_real_analysis.md")
...
if __name__ == '__main__':
    ...
    print(f"exp-016: Semi-Real CM with Better Model (gpt-3.5-turbo)")
    print(f"N={N}, n_mc={n_mc}, n_experts={N_EXPERTS}")
    print(f"Subgroup CMs (real asymmetric from gpt-3.5-turbo pilot):")
    for s in SUBGROUP_LABELS:
        cm = SUBGROUP_CM[s]
    ...
    df_raw.to_csv('results/exp016_semi_real_raw.csv', index=False)
    print(f"Saved results/exp016_semi_real_raw.csv ({len(df_raw)} rows)")
