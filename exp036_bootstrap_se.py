#!/usr/bin/env python3
"""exp036_bootstrap_se.py — Bootstrap SE for CausalForest coverage improvement

Tests whether bootstrap SE can close the CausalForest under-coverage gap
(~91% vs nominal 95%). Each bootstrap iteration re-estimates CM and re-runs
CausalForest.

Setup:
  - K=2, N=5000, n_expert=250, pi=[0.05, 0.25]
  - CausalForest via econml CausalForestDML (discrete_treatment=True)
  - Bootstrap B=200, re-estimating CM + CF per bootstrap sample
  - Compare plug-in SE vs bootstrap SE coverage
  - 30 MC runs (reduced due to computational cost)

Output:
  results/exp_bootstrap_se.csv
  results/exp_bootstrap_se_summary.csv
"""
...
def fit_cf(Y, T, X_feat, seed):
    """Fit CausalForest and return fitted model."""
    cf = CausalForestDML(
...
def get_cf_subgroup_estimates(cf, X_feat, Z_labels, S):
    """Get per-(subgroup, z_level) CATE estimates from fitted CF."""
    results = {}
    for s in range(K):
        for z in [0, 1]:
            ...
            tau_hat = te.mean()
            # Plug-in SE: try effect_inference first
            try:
...
def run_one_mc(seed):
    ...
    # Build features: Z_hat and S for heterogeneity discovery
    ...
    # Apply CM correction for plug-in
    plugin_corrected = {}
    for s in range(K):
...
def main():
    ...
    print(f"Bootstrap SE for CausalForest: K={K}, N={N}, n_expert={N_EXPERT}")
    ...
    for mc in range(N_MC):
        mc_t0 = time.time()
        print(f"  MC {mc}/{N_MC} ...", end='', flush=True)
    ...
    df.to_csv('results/exp_bootstrap_se.csv', index=False)
    print(f"\nWrote {len(df)} rows to results/exp_bootstrap_se.csv")
    ...
    summary.to_csv('results/exp_bootstrap_se_summary.csv', index=False)
    ...
    sg_summary.to_csv('results/exp_bootstrap_se_subgroup.csv', index=False)

    print("\n" + "=" * 80)
    print("BOOTSTRAP SE — COVERAGE COMPARISON")
    print("=" * 80)
    print(summary.to_string(index=False))

    print("\n" + "=" * 80)
    print("BOOTSTRAP SE — PER-SUBGROUP")
    print("=" * 80)
